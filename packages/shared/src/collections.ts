...
function stateCollection(abbr: string): CollectionDef {
  ...
  return {
    ...
    body: `<p>The Social Security Administration publishes births by state, which makes it possible to find names that never left one. Each name here has at least ninety percent of its recorded births in ${full}, with enough volume that the concentration is not an artifact of the per-state reporting floor.</p><p>Some are family traditions that stayed local. Some track a place, an institution, or a community with a hard geographic boundary.</p>`,
    group: "geography",
    columns: ["metric", "peak", "total", "spark"],
    ...
    select: (facts) =>
      take(
        ...
        (f) => `${Math.round((f.top_state_share ?? 0) * 100)}% of all births`,
