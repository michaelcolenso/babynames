...
export default {
  ...
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/compute-diaspora") {
      ...
      let names = 0;
      do {
        const r = await computeDiasporaChunk(env.DB, cursor, DIASPORA_MAX_PAGES, totals);
        names += r.namesDone;
        cursor = r.nextCursor;
      } while (cursor);
...
async function handleMessage(env: Env, msg: IngestMessage): Promise<void> {
  switch (msg.type) {
    ...
    case "diaspora-finalize": {
      ...
      // Denominators are small (~6k rows) and cheap to reload per message; the
      // chain spans many messages so we can't thread them through state.
      const totals = await loadStateYearTotals(env.DB);
