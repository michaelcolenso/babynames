...
export function computeDecadeCoverage(source: DecadeHubSource, config: DecadeComputeConfig): DecadeCoverage {
  if (source.minYear > config.startYear) throw new Error(`source min year ${source.minYear} is after decade start ${config.startYear}`);
...
export function evaluateSanityAnchors(source: DecadeHubSource, config: DecadeComputeConfig): AnchorReport {
  ...
  for (const anchor of config.sanityAnchors) {
    ...
    else if (anchor.kind === "record-year-count") { if (anchor.year < coverage.startYear || anchor.year > coverage.endYear) throw new Error(`sanity anchor year ${anchor.year} is outside actual coverage ${coverage.startYear}–${coverage.endYear}`); const value = find(anchor.name, anchor.sex)?.series[anchor.year] ?? 0; report[`${anchor.name}${anchor.sex}${anchor.year}`] = value; if (value < anchor.min || value > anchor.max) throw new Error(`sanity anchor failed: ${anchor.name} (${anchor.sex}) ${anchor.year} = ${value}, expected ${anchor.min}–${anchor.max}`); }
