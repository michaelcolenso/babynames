...
import { loadDecadeHubRuntime, renderDecadeMethodologyGeneric } from "@nv/shared";
...
function notFound(decade: string): Response {
  return new Response(
    `<!doctype html><html><body><h1>Not found</h1><p>No methodology page exists for ${escapeHtml(decade)}.</p></body></html>`,
...
export const onRequestGet: PagesFunction<Env, "decade"> = async (ctx) => {
  ...
  return new Response(renderDecadeMethodologyGeneric(runtime.profile, { origin, definition: runtime.definition, thesis: runtime.thesis }), {
    headers: {
