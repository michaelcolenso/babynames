// Pure compute functions for the 1980s decade hub (SPEC §3–§6).
 ...
 * construction — on real 1984 data no name reaches a full expected seat
 * (Michael's expected seats ≈ 0.60), so the real roster is all-unique; that is
 * a documented property of the 1984 distribution, not a uniqueness constraint.
 */
...
export interface AnchorReport {
  totalBirths1984: number;
  michaelM1984: number;
  jenniferF1980s: number;
}

/**
 * Sanity anchors. NOTE (deviation flagged): SPEC §1 states "1984 total births
 * ≈ 3.66M ± 2%". 3.66M is the *registered* US birth count for 1984; the SSA
 * public files (and the tracked shards derived from them) sum only names with
 * >= 5 occurrences, which totals ~3.49M for 1984 (verified against an
 * independent 2024-vintage mirror: 3,488,755). This function therefore asserts
 * the published-count expectation (3.49M ± 3%) plus the SPEC's other anchors.
 */
export function assertSanityAnchors(source: DecadeHubSource): AnchorReport {
  let total1984 = 0;
  let michaelM1984 = 0;
  let jenniferF1980s = 0;
  for (const rec of source.records) {
    total1984 += rec.series[CLASSROOM_YEAR] ?? 0;
    if (rec.sex === "M" && rec.name === "Michael") michaelM1984 = rec.series[CLASSROOM_YEAR] ?? 0;
    if (rec.sex === "F" && rec.name === "Jennifer") {
      for (let y = DECADE_START; y <= DECADE_END; y++) jenniferF1980s += rec.series[y] ?? 0;
    }
  }
  const expected1984 = 3.49e6;
  const tol = 0.03;
  if (Math.abs(total1984 - expected1984) / expected1984 > tol) {
    throw new Error(`sanity anchor failed: 1984 total births ${total1984} outside ±3% of ${expected1984}`);
  }
  if (michaelM1984 < 60000 || michaelM1984 > 70000) {
    throw new Error(`sanity anchor failed: Michael (M) 1984 = ${michaelM1984}, expected 60k–70k`);
  }
  if (jenniferF1980s <= 350000) {
    throw new Error(`sanity anchor failed: Jennifer (F) 1980s = ${jenniferF1980s}, expected > 350k`);
  }
  return { totalBirths1984: total1984, michaelM1984, jenniferF1980s };
}
