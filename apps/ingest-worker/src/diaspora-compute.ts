...
// Ignore one-off state rows until the name is nationally visible enough to
// have a real wave. Names that never cross this floor fall back to literal
// first state appearances, preserving genuinely local names.
export const NATIONAL_VISIBILITY_FLOOR = 10; // national rate per 100k same-sex births
...
export type SexYearTotals = Map<Sex, Map<number, number>>;

function sexYearTotal(totals: SexYearTotals | undefined, sex: Sex, year: number): number {
  return totals?.get(sex)?.get(year) ?? 0;
}
...
export function computeDiasporaForName(
  ...
  let nationalVisibleFrom: number | null = null;
  if (sex && totals) {
    for (const [year, nameNational] of [...nameNationalByYear.entries()].sort((a, b) => a[0] - b[0])) {
      const totalBirths = sexYearTotal(totals, sex, year);
      if (totalBirths <= 0) continue;
      const rate = (nameNational / totalBirths) * 100_000;
      if (rate >= NATIONAL_VISIBILITY_FLOOR) {
        nationalVisibleFrom = year;
        break;
      }
    }
  }
  ...
  for (const r of rows) {
    if (r.count < THRESHOLD) continue;
    if (nationalVisibleFrom !== null && r.year < nationalVisibleFrom) continue;
...
export async function loadSexYearTotals(db: D1Database): Promise<SexYearTotals> {
  const r = await db
    .prepare("SELECT sex, year, SUM(count) AS births FROM name_states GROUP BY sex, year")
    .all<{ sex: Sex; year: number; births: number }>();
  const totals: SexYearTotals = new Map();
  for (const row of r.results ?? []) {
    let byYear = totals.get(row.sex);
    if (!byYear) {
      byYear = new Map();
      totals.set(row.sex, byYear);
    }
...
function buildDiasporaStatements(
  ...
  for (let i = 0; i < page.length; i += ROWS_PER_STMT) {
    ...
    for (const agg of slice) {
      const d = computeDiasporaForName(agg.rows, agg.sex, totals);
