...
// Naming diversity metrics per year and sex (HHI, top-1/top-10 share, unique names).
//
...
import { getMeta, getVizPayload, META_KEYS, type ConcentrationResponse } from "@nv/shared";
...
export const onRequestGet: PagesFunction<Env> = async (ctx) => {
  ...
  const body = await getVizPayload<ConcentrationResponse>(ctx.env.DB, "concentration", dataVersion ?? "", ym, yM);
