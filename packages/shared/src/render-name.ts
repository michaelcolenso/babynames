...
function diasporaNarrative(d: DiasporaResponse, name: string): string {
  ...
  if (d.origin.year === 1910) {
    return `${safe} was already present when SSA state-level records begin in 1910; the earliest state data shows it in ${origin}.`;
  }
  if (d.totalStates <= 1) {
...
function renderDiasporaMap(record: NameRecord, d?: DiasporaResponse): string {
  ...
  const tiles = ALL_STATES.map((st) => {
    ...
    const titleYear =
      tier === "never"
        ? "not recorded"
        : originYear === 1910 && adoptedYear === 1910
          ? "present in earliest state data"
          : `first recorded ${adoptedYear}`;
  ...
  const subtitle = d.origin
    ? d.origin.year === 1910
      ? `Earliest state data: <strong>${originLabel}, 1910</strong> · present in ${d.totalStates} ${d.totalStates === 1 ? "state" : "states"} by ${d.origin.year + d.diffusionYears}`
