...
import { fetchDecadeHubProfile, renderDecadeMethodology } from "@nv/shared";
import type { PagesFunction } from "@cloudflare/workers-types";

function notFound(decade: string): Response {
  return new Response(
    `<!doctype html><html><body><h1>Not found</h1><p>No methodology page exists for ${decade}. The decade hub currently ships for the <a href="/names/1980s/">1980s</a> only.</p></body></html>`,
...
export const onRequestGet: PagesFunction<Env, "decade"> = async (ctx) => {
  ...
  const canonical = `${origin}/names/1980s/methodology/`;
  return new Response(renderDecadeMethodology(profile, { origin }), {
    headers: {
