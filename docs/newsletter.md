...
- **Per-address limit**: 3 confirmation sends / 24 h, with its own message — the two windows differ by two orders of magnitude, so telling a day-blocked address to retry "in a few minutes" just buys more failures. The honeypot and IP limit both key on the *sender*, which does nothing to stop a rotating address pool aiming the signup form at a victim's inbox. This one keys on the recipient.
...
- **Stale pending rows** are swept from the ingest worker's existing weekly cron (it shares this database), *and* opportunistically on subscribe. The cron is what makes the guarantee hold on a quiet week — the opportunistic sweep alone would leave a row waiting for the next signup that may never come. Deleted once their confirmation window closes — an expired token can never activate the row, so keeping it would retain an address nobody finished consenting to (and the confirmation email promises otherwise). Swept beside the rate-limit buckets.
