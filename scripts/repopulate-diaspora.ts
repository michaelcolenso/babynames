...
async function loadSexYearTotals(): Promise<SexYearTotals> {
  const rows = await q<{ sex: "M" | "F"; year: number; births: number }>(
    "SELECT sex, year, SUM(count) AS births FROM name_states GROUP BY sex, year",
  );
  const totals: SexYearTotals = new Map();
  for (const r of rows) {
    let byYear = totals.get(r.sex);
    if (!byYear) {
      byYear = new Map();
      totals.set(r.sex, byYear);
    }
...
async function insertBatch(aggs: NameAgg[], peaks: Map<string, number>, totals: SexYearTotals) {
...
async function main() {
  console.log(`Loading sex-year totals...`);
  const totals = await loadSexYearTotals();
  ...
  for (;;) {
    ...
    if (DRY_RUN) {
      ...
      await insertBatch(page, peaks, totals);
    }
    done += page.length;
