...
export function OwnershipTable(rows: OwnershipResult[], opts: { caption:string; showSex?:boolean }): string { return OwnershipTableGeneric(rows, { ...opts, period:"1980s" }); }
...
export async function fetchDecadeHubProfile(db: D1Database, decadeLabel = "1980s"): Promise<DecadeProfile | null> { try { const row=await db.prepare("SELECT payload FROM decade_hub WHERE decade = ?1").bind(decadeLabel).first<{payload:string}>(); if(!row || typeof row.payload !== "string") return null; const profile=JSON.parse(row.payload) as DecadeProfile; return profile?.decade === DEFINITION.startYear && profile.ownershipRankings && profile.classroomDefaults && Array.isArray(profile.spellingFamilies) ? profile : null; } catch { return null; } }
export function renderDecadeHub(profile: DecadeProfile, opts: DecadePageOpts): string { return renderDecadeHubGeneric(profile, { ...renderOptions(opts), decadeNavigation:"adjacent-only" }); }
export function renderDecadeClassroom(profile: DecadeProfile, opts: DecadePageOpts): string { return renderDecadeClassroomGeneric(profile, renderOptions(opts)); }
export function renderDecadeSpellingFamilies(profile: DecadeProfile, opts: DecadePageOpts): string { return renderDecadeSpellingFamiliesGeneric(profile, renderOptions(opts)); }
export function renderDecadeMethodology(profile: DecadeProfile, opts: DecadePageOpts): string { return renderDecadeMethodologyGeneric(profile, renderOptions(opts)); }
