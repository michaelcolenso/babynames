...
import { fetchDecadeHubProfile, renderDecadeSpellingFamilies } from "@nv/shared";
import type { PagesFunction } from "@cloudflare/workers-types";

function notFound(decade: string): Response {
  return new Response(
    `<!doctype html><html><body><h1>Not found</h1><p>No spelling-families page exists for ${decade}. The decade hub currently ships for the <a href="/names/1980s/">1980s</a> only.</p></body></html>`,
...
export const onRequestGet: PagesFunction<Env, "decade"> = async (ctx) => {
  ...
  const canonical = `${origin}/names/1980s/spelling-families/`;
  return new Response(renderDecadeSpellingFamilies(profile, { origin }), {
    headers: {
