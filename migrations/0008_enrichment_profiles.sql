-- Enrichment System: precomputed dossier layers for /name/:name.
--
-- All four tables are additive and keyed by (name_lower, sex) to match the
-- names.name_lower convention. Heavy computation (survival curves, weighted
-- std-dev, Location Quotient) happens offline in scripts/build-enrichment.ts;
-- request-time code only reads these rows.
