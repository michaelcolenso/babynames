...
const FACTS_COLUMNS = [
  ...
  "spike_year", "spike_ratio", "spike_baseline",
...
function factsTuple(r: NameFacts): string {
  return `(${[
    ...
    nOrNull(r.spike_year), nOrNull(r.spike_ratio), nOrNull(r.spike_baseline),
...
export function emitSql(
  ...
  // Staleness markers. facts_version copies whatever data_version is live at
  // seed time, so a later ingest (which mints a fresh data_version UUID) makes
  // the two diverge and verify-name-facts.ts fails on the drift. Stamping it
  // from the database rather than from a CLI argument means it cannot be wrong.
  out.push(
    "INSERT INTO meta (key, value) SELECT 'facts_version', value FROM meta WHERE key = 'data_version'" +
 ...
 */
function accumulateStateTotals(zipBytes: Uint8Array): Map<string, Map<string, number>> {
  const totals = new Map<string, Map<string, number>>();
  let rowCount = 0;
  forEachStateRow(zipBytes, (state, sex, _year, name, count) => {
    rowCount++;
    const key = `${name}|${sex}`;
  ...
  console.error(`State corpus: ${rowCount.toLocaleString()} rows over ${totals.size} (name,sex) pairs`);
  return totals;
}
...
async function main(): Promise<void> {
  ...
  const stateTotals = flag("no-state")
    ? undefined
  ...
  const rows = buildFactsRows(series, national.yM, {
    ...
    sourceDataVersion: arg("data-version") ?? null,
    catalysts,
    stateTotals,
  });
  ...
  const sql = emitSql(rows, members);
